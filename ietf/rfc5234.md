...
它权衡了紧凑性和简单性并具有合理的表征能力。在阿帕网 (Arpanet) 早期，每个规范都包含了各自的 ABNF 定义。
...
标准 BNF 和 ABNF 之间的区别涉及命名规则，重复 (repetition)，选择 (alternative)，顺序独立，和值范围。

[附录 B](#appendix-B) 提供了对几种因特网规范常见的核心词汇分析器的规则定义和编码。

它提供了便利的方式并独立于文档正文中定义的元语言，且与其正式地位相独立。

*注: [元语言](https://zh.wikipedia.org/wiki/%E5%85%83%E8%AA%9E%E8%A8%80)*
# 规则定义
...
这通常局限于在自由形式的文档中使用的规则名称引用，或者用于区分部分没有使用空白符而组成一个字符串的规则，
...
- name是不区分大小写的非终结符号
- "CRLF"表示回车换行(ascii \r\n)
- [终结符](https://zh.wikipedia.org/wiki/%E7%B5%82%E7%B5%90%E7%AC%A6%E8%88%87%E9%9D%9E%E7%B5%82%E7%B5%90%E7%AC%A6)

\<name> 是规则的名称，\<elements> 是一个或多个规则名称或终结符 (terminal)，\<crlf> 是行结束指示符（回车后紧跟着换行）。

等于符号用于将规则的定义和名称分隔开。

elements 构成了一个或多个规则名(以及/或者)值定义的序列，并组合了本文档中定义的各种操作符，比如选择和重复。
...
允许直线空白（space和tab符）能够自由地，隐式地散落在主要结构中，比如界定符或原子字符串
